...
public class Server {
	...
	public Object callHttp(Object json) throws MalformedURLException, IOException {
		...
		String url = prefix + jsonObject.get(MogawayContants.PARAM_URL).getAsString();
		String method = (jsonObject.has(MogawayContants.PARAM_METHOD)) ? jsonObject.get(MogawayContants.PARAM_METHOD).getAsString() : "GET";
		...
		if(MogawayContants.REQUEST_METHOD_POST.equals(method)){
			...
			if(jsonObject.has(MogawayContants.PARAM_CONTENT_TYPE)){
				...
				httpConnection.setRequestProperty("Content-Type", jsonObject.get(MogawayContants.PARAM_CONTENT_TYPE).getAsString());
				if(jsonObject.has(MogawayContants.PARAM_DATA)){
					JsonObject data = jsonObject.get(MogawayContants.PARAM_DATA).getAsJsonObject();
				...
				if(jsonObject.has(MogawayContants.PARAM_DATA)){
					...
					while(it.hasNext()){
						...
						if(it.hasNext()) postPayload.append("&");
					}
				}
			}
